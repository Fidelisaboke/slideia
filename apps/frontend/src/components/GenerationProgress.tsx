      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="mb-6 relative">
            <motion.div
              animate={{
                rotate: currentStep === 'complete' || currentStep === 'error' ? 0 : 360,
                scale: currentStep === 'complete' || currentStep === 'error' ? [1, 1.2, 1] : 1,
              }}
              transition={{
                rotate: { 
                  duration: currentStep === 'complete' || currentStep === 'error' ? 0.3 : 8, 
                  repeat: currentStep === 'complete' || currentStep === 'error' ? 0 : Infinity, 
                  ease: currentStep === 'complete' || currentStep === 'error' ? "easeOut" : "linear" 
                },
                scale: { duration: 0.5 }
              }}
          ...
          {/* Main Status */}
          <h3 className={`text-xl font-bold mb-2 font-(family-name:--font-sora) ${currentStep === 'error' ? 'text-destructive' : 'text-foreground'}`}>
            {currentStep === 'complete' ? 'Presentation Ready!' : 
             currentStep === 'error' ? 'Generation Failed' : 'Generating Magic...'}
          </h3>
          ...
          <div className="w-full h-3 bg-muted rounded-full overflow-hidden mb-4 p-[2px] border border-border/50">
            <motion.div
              className={`h-full rounded-full ${
                currentStep === 'complete' ? 'bg-secondary' : 
                currentStep === 'error' ? 'bg-destructive' : 'bg-linear-to-r from-primary to-secondary'
              }`}
              initial={{ width: '0%' }}
              animate={{ width: currentStep === 'error' ? '100%' : `${progress}%` }}
          ...
          <div className="flex justify-between w-full text-[10px] uppercase tracking-widest font-bold text-muted-foreground/60 mb-8">
            <span>
              {currentStep === 'slide' && totalSlides 
                ...
                : currentStep === 'outline' ? 'Structuring' : 
                  currentStep === 'complete' ? 'Done' : 
                  currentStep === 'error' ? 'Error' : 'Thinking'}
            </span>
            <span>{currentStep === 'error' ? '!' : `${Math.round(progress)}%`}</span>
          </div>
          ...
          {currentStep === 'slide' && slideTitle && (
            ...
            >
              <span className="text-[10px] text-muted-foreground block mb-1">CURRENTLY DRAFTING</span>
              <p className="text-sm font-medium text-foreground truncate">{slideTitle}</p>
            </motion.div>
          )}
